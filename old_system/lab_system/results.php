...
// Function to save test results
 // Function to save test results
function saveTestResults($template_id, $patient_id, $field_values, $test_date, $test_time) {
    global $conn;
    
    $patient_id = (int)$patient_id;
    
    // Sanitize the date and time inputs
    $safe_test_date = mysqli_real_escape_string($conn, $test_date);
    $safe_test_time = mysqli_real_escape_string($conn, $test_time);

    // MODIFIED: The SQL now inserts into both test_date and the new test_time column
    $sql = "INSERT INTO ".$_SESSION['my_tables']."_laboratory.test_results (template_id, patient_id, test_date, test_time)
            VALUES ($template_id, $patient_id, '$safe_test_date', '$safe_test_time')";
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        foreach ($field_values as $field_id => $value) {
            ...
            // Handle empty values by inserting NULL
            if ($value === '') {
                $value_sql = "NULL";
            } else {
            ...
            $sql_values = "INSERT INTO ".$_SESSION['my_tables']."_laboratory.result_values (result_id, field_id, field_value)
                           VALUES ($result_id, $field_id, $value_sql)";
